...
#Gemini is the primary embedding model, and we probe it first. If it fails, we fall back to sentence-transformers.
def _probe_gemini():
...
#For fallback, we use the sentence-transformers library with the all-mpnet-base-v2 model.
def _load_fallback():
...
#To initialize the embedding model once per process.
def _init():
...
#For embedding a single query string, i.e the user query converted into a vector representation
...
# For embedding a list of text strings, i.e the documents converted into vector representations
